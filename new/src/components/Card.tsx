...
export default function Card({ card, onToggle, onRemove }: CardProps) {
  ...
  return (
    <div className="p-5 rounded-3xl  bg-white dark:bg-[#21212C]/90 dark:text-white flex flex-col justify-between gap-9 shadow-sm shadow-[#545969]">
      <div className="flex items-center gap-2 md:flex-wrap">
        <img src={card.icon} alt={`${card.title} icon`} className="w-17 h-17 pb-[12px] pr-2" />
        <div className="flex flex-col flex-wrap w-65">
          <h2 className="font-bold text-2xl">{card.title}</h2>
        <p className="text-[16px] font-bold text-[#545969] dark:text-[#C7C7C7]">{card.description}</p>
        </div>
