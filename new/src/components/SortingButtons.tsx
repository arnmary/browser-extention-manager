...
export default function SortingButtons({activeFilter, setActiveFilter}: SortingButtonsProps) {
  ...
  return (
    <div className="font-[2em] flex flex-row items-center  sm:items-end justify-center sm:justify-end gap-3.5">
      {buttons.map((btn)=>(
        <button
        key={btn}
        className={`text-base text-center rounded-4xl px-[26px] py-[11px] shadow-sm cursor-pointer transform transition-all duration-300 ease-in-out ${
            activeFilter === btn
